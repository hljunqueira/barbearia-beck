...
export const OpeningHours = ({
  ...
  const activeHours = (hours && hours.length > 0 ? hours : SITE.hours).filter((slot) => slot.open);
  const title = walkInTitle || SITE.slogans.walkIn;
  const subtitle = walkInSubtitle || 'Atendimento por ordem de chegada — sem hora marcada';
  const targetMapsUrl = mapsUrl || mapsLink();
  const addressDisplay = fullAddressText || fullAddress();
